...
1. Links `zsh/.zshrc` and `zsh/.zsh/` to `~/`.
...
Keep machine-local secrets and private endpoints out of git by creating a real file in the linked `~/.zsh/` directory, for example `~/.zsh/zz-local-env.zsh`:
