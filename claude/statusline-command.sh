...
# Find VCS root and check for disable marker (touch .git/claude-no-vcs etc.)
vcs_root=""
if git -C "$current_dir" rev-parse --show-toplevel >/dev/null 2>&1; then
  vcs_root=$(git -C "$current_dir" rev-parse --show-toplevel 2>/dev/null)
elif jj root --ignore-working-copy -R "$current_dir" >/dev/null 2>&1; then
  vcs_root=$(jj root --ignore-working-copy -R "$current_dir" 2>/dev/null)
elif hg -R "$current_dir" root >/dev/null 2>&1; then
  vcs_root=$(hg -R "$current_dir" root 2>/dev/null)
fi

if [ -n "$vcs_root" ] && { [ -e "$vcs_root/.git/claude-no-vcs" ] || [ -e "$vcs_root/.jj/claude-no-vcs" ] || [ -e "$vcs_root/.hg/claude-no-vcs" ]; }; then
  vcs_seg="$(bg $GREEN)$(fg $prev_color)${SEP}$(bg $GREEN)$(fg $CRUST)\e[1m  vcs off $(reset)"
  prev_color="$GREEN"
