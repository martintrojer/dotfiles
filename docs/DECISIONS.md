...
- **The one subtlety — folding:** the repo stows everything `--no-folding` (per-leaf) to avoid the shared-target fold bug. But skills *must* fold so each skill links as one opaque directory symlink — otherwise stow descends per-file and `.stowrc`'s global `README.md`/`LICENSE.*` ignore drops `unslop/LICENSE` and `detector/README.md`. So `skills` opts back into folding via `PackageSpec.fold`, and a `fold_anchors` mkdir (`~/.agents/skills/`) pins the fold level (without a pre-existing real dir, stow folds `~/.agents` into a single repo symlink). Pi extensions are all `.ts`, so the ignore never fires and they stow per-file under plain `--no-folding`.
...
### Waybar is a restrained statusline, not a widget strip (accepted 2026-05-23; geometry revised 2026-09-20)

Waybar should visually read as the topmost statusline of the Sway/tmux/nvim stack. The old slash/backslash spacer layout made it feel like floating text with decorative separators. The current design uses compact rounded pills only for navigation, selection, and states that need attention. Weather and the clock remain plain text on the translucent bar.

Several more colorful variants were tried first: full pastel fills, then high-alpha pastel cluster backgrounds, then only selected pastel clusters. Dark text on pastel blocks was very legible, but it reduced refinement and fought the terminal-first desktop. The accepted compromise is deliberately bland: surfaces for structure, lavender only for the active workspace, red only for warnings and offline states. Rounded geometry now matches Swayward's windows without turning every module into a widget.

The custom spacer modules were removed because CSS margins provide separation without extra config objects. Unused modules (`sway/mode`, `battery`, `backlight`) were also removed because they were configured but not displayed. `sway/mode` can return if Sway binding modes become part of the workflow; today resize is done through direct keybindings.
...
**Superseded in scope 2026-08-28 — the multi-machine question, and `murmur`.** The entries above argue a single-machine trade: modal picker versus permanent sidebar, on one host. The question that actually matters turned out to be a different one — *every agent on every machine, in one view* — and nothing on offer answers it. Design lives in [`~/hacking/murmur`](https://github.com/martintrojer/murmur); this entry records why it is a separate tool and what leaves this repo when it ships.

- **Neither candidate solves it, and herdr structurally cannot soon.** Verified against installed herdr 0.8.2: `--remote` takes one target, and no `agent`/`pane`/`notification` subcommand has a `--host` flag. It is 1:1 by construction. The maintainer calls multi-client "top priority" ([discussion #515](https://github.com/herdrdev/herdr/discussions/515), 2026-07-13) but gated behind a server/client refactor that `AGENTS.md` still describes as in progress six weeks later. T3 Code has better remote *access* (bearer, relay, Tailscale, SSH) but its own `docs/internals/remote.md` lists "richer multi-environment UI" as unbuilt — and it has no pi driver, which is a 14-method adapter (`OpenCodeAdapter.ts` is 1741 lines) in a repo not accepting big features.
- **Waiting for herdr would be a downgrade even if it landed.** Its scope is one client attaching to many *herdr* servers, so every machine must run herdr — including the Meta devvm, where `mu` then could not run (`allow_nested = false`). And herdr infers state by matching terminal output. This repo already pushes authoritative state from inside pi. Adopting herdr means trading better data for worse.
- **Why a separate repo rather than more `tmux/` scripts.** `agent-attention` is 1511 lines of Python living in a config directory, fed by two other packages (`pi/`, `opencode/`), referenced from six docs, with no test file of its own. It is a tool wearing a config file's clothes. The forcing move is deployment: the moment a second machine must export state, an installable artifact is required, and dotfiles cannot be deployed to a corporate devvm. Any design here produces a distributable tool; the only question is whether it admits it.
- **TypeScript, on install-story grounds.** pi is an npm package, so every node running agents already has Node — npm is not a new dependency on exactly the machines that matter, and `@martintrojer/mu` proves the shape. Python has no global install one would trust across a Mac, a Fedora box, and a devvm where system Python is least ours to touch. The "keep 1511 working lines" argument is weaker than it looks: those lines work because dotfiles symlink them, so a remote node means packaging from scratch either way.
- **What leaves and what stays.** Out: `agent-attention`, the agent half of `_tmux_common`, `scan_agent_states`, the fzf picker plumbing. In: the `prefix+a` bind, `@agent_glyphs` theming (still catppuccin via `docs/palette.toml` and `make theme`), and a thin `status-ai` consuming `murmur status --json`. The line is *tool owns behaviour, dotfiles own appearance and keys* — the same boundary `tms` drew for multiplexer backends.
- **One prerequisite is a dotfiles change on its own merits.** `ssh/.ssh/config` has no `ControlMaster` at all, so every `ssh dev` costs a yubikey touch. A control socket makes "adhoc ssh" and "open tunnel" the same code path, and lets a background collector ride an existing authenticated connection without ever prompting. herdr reached the same conclusion and ships its own per-attach control socket; stock OpenSSH config is enough here.
- **The pillar #11 read, applied to ourselves.** Rejecting herdr partly for being young single-maintainer infra and then writing a young single-maintainer tool needs an answer. It is the substrate/work asymmetry from the entry above: murmur is not the multiplexer. tmux still owns the panes, and murmur owning the answer to "what is every agent doing" is a script-tier bet — reviewable, testable, revertible in one command. If it dies, `prefix+a` gets worse; nothing stops working.

**Shipped 2026-08-29.** murmur is installed and owns agent state; `agent-attention` and its pi extension are deleted. Three deltas from the plan above worth recording, because each was only visible once the seam was real:

- **`status-ai` parses `murmur status`, not `--json`.** Both exist. The plain `<state>\t<count>` form is what a status renderer wants — no JSON parse on a path that runs every tmux tick — and validating a closed vocabulary out of two tab-separated fields is a smaller sink for the `#{E:...}` re-expansion hazard.
- **The `prefix + a` filter keys went away with the script.** `murmur pick` has no `ctrl-a/b/w/d/x` state filters; it takes `--all` for orchestrated agents and owns its own UI. Docs promising the old key map outlived the script that implemented it.
- **`@agent_state` has a second consumer that outlived the migration.** `tms` colours its session rows from it via `_tmux_common.scan_agent_states`, so the agent half of `_tmux_common` did *not* leave as planned — `AgentState`, `AgentStats`, `STATE_GLYPH` and `scan_agent_states` all stayed. "Out: the agent half of `_tmux_common`" was too strong.
