...
**Reconsider if:** a real fleet (4–6 agents, 3+ workspaces) makes the agent panel pay for itself, OR herdr gains a movable/horizontal panel, OR `mu` stops being the daily driver for orchestration. The package stays in the repo so any of those is a config edit, not a rebuild.
