#!/usr/bin/env bash
# Emoji picker using bemoji
# Source: https://github.com/marty-oehme/bemoji

# Force fuzzel as picker
export BEMOJI_PICKER_CMD="fuzzel -d -p 'Emoji ' --font Twemoji"
...
dl_default_emoji() {
    local emojis
    emojis=$(curl -sSL "https://unicode.org/Public/emoji/16.0/emoji-test.txt")
...
_typer() {
    ...
    if [ -n "$WAYLAND_DISPLAY" ]; then
        if command -v wtype >/dev/null 2>&1; then
            wtype -s 30 "$totype"
        elif command -v wl-ime-type >/dev/null 2>&1; then
            wl-ime-type "$totype"
        fi
...
result=$(gather_emojis 2>/dev/null | _picker)
exit_value="$?"
